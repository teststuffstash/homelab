...
|---|---|---|---|---|
| **Garage (S3)** | 🟢 LIVE | S3-compatible object store | `https://s3.teststuff.net` (region `garage`, path-style) · `garage.garage.svc:3900` · [SLO dashboard](https://grafana.teststuff.net/d/garage-slo/) · [upstream raw-rates dashboard](https://grafana.teststuff.net/d/garage-upstream/) | app-owned buckets → [pattern](docs/patterns/app-owned-resources.md), [`docs/garage.md`](docs/garage.md); ADR-031/073/074/075. `argocd/platform/garage.yaml` (migrated off tofu 2026-08-04); ⚠ no offsite backup — FU-137 |
| **Longhorn** | 🟢 LIVE | Replicated block storage, four tiers (ADR-089 + its 2026-08-07 addendum): `longhorn` (default, **std** 2-replica; ⚠ **TWO schedulable zones since 2026-09-12** — m70s + hp-01, after thinkcentre left cluster duty and wk-02's pooled disk was fenced. 732G allocatable / 147G committed, so a new 2-replica volume has room, but both copies of every volume now live on those two nodes: no third zone to rebuild onto, and soft anti-affinity fails as silent co-location — `docs/storage-ledger.md`), `longhorn-bulk` (**bulk** 2-replica across wk-metal-01 + wk-metal-04 — 706G allocatable but ≈90% committed: Garage data 150Gi and the deliberately-oversized registry mirrors take most of it, so **a new bulk grant is a capacity decision, ask first**), `longhorn-fast` (Optane scratch, replica=1 — ⚠ **NO backing disk since 2026-09-12**: the pair left with thinkcentre, so a PVC on this class stays Pending until they land on wk-metal-04, FU-234), `longhorn-scratch` (per-ride throwaway, replica=1 on the bulk disks — the docker-ride dind block PVCs, FU-081; transient ≤20Gi×3 per docker repo, now quota-capped) | StorageClasses `longhorn`, `longhorn-bulk`, `longhorn-fast`, `longhorn-scratch` | PVCs, capped per namespace by the AgentStack claim's `storage` block; ADR-030/089 |
...
| **PyPI cache (pull-through)** | 🟢 LIVE | nginx mirror of `pypi.org/simple/` (index pages, short TTL) + `files.pythonhosted.org` (immutable wheels/sdists, long TTL) on a `longhorn-bulk` PVC — makes agent-worker sandbox python installs LAN-speed and WAN-independent (the FU-196 argument, transplanted) — ⚠ **unlocked paths only** (`uv pip install`, `uvx`, `uv run --with`, pip): a project's `uv sync --frozen` installs from the URLs its committed lock carries, so it bypasses the cache (FU-220) | in-cluster `pypi-cache.pypi-cache.svc`; BGP VIP `192.168.40.34` (kata-reachable) | consumer wiring: #1413 (launcher env plumbing) — sandbox rides set `UV_DEFAULT_INDEX=http://192.168.40.34/simple/` **and `UV_FROZEN=1` with it** (uv records the index in `uv.lock`, so the pair is one decision — [`docs/patterns/python-stack.md`](docs/patterns/python-stack.md) §caches); `proxy_cache` two zones — `/simple/` 5m TTL, `/packages/` 30d TTL; homelab#1300, `argocd/resources/pypi-cache/` |
...
- **Storage (Longhorn):** request a PVC with `storageClassName: longhorn` (small, replicated),
  `longhorn-bulk` (large volumes: S3 data, backups, datasets) or `longhorn-fast` (Optane SCRATCH: single-node replica-1, modest speed — intent is disk-write-heavy pods (CI builds), NEVER load-bearing data/metadata; FU-159 ruling 2026-08-11. ⚠ Unusable until FU-234 re-homes the drives — do not plan a consumer on it yet).
