...
| **PyPI cache (pull-through)** | 🟢 LIVE | nginx mirror of `pypi.org/simple/` (index pages, short TTL) + `files.pythonhosted.org` (immutable wheels/sdists, long TTL) on a `longhorn-bulk` PVC — makes agent-worker sandbox `uv sync` LAN-speed and WAN-independent (the FU-196 argument, transplanted) | in-cluster `pypi-cache.pypi-cache.svc`; BGP VIP `192.168.40.34` (kata-reachable) | consumer wiring: #1413 (launcher env plumbing) — sandbox rides set `UV_DEFAULT_INDEX=http://192.168.40.34/simple/`; `proxy_cache` two zones — `/simple/` 5m TTL, `/packages/` 30d TTL; homelab#1300, `argocd/resources/pypi-cache/` |
