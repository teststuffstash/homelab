...
# baked ABOVE this gate. `bash` is shadowed as the subscription-latch seam: for this fixture it
# returns 1 with empty stdout — the ladder's BOTH-LATCHED verdict. `curl` is shadowed so a stray
# Go probe REDS loudly instead of hanging offline.
HARNESS="claude"
MODEL="haiku"
