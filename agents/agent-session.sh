...
render_env_card() {
  local mdio="${AGENT_MIRROR_DOCKER_IO-http://192.168.40.20}" mghcr="${AGENT_MIRROR_GHCR-http://192.168.40.21}" ncache="${AGENT_NIX_CACHE_URL-http://192.168.40.23}"
  ...
  printf '%s\n' "- **Package proxies — upstream is egress-blocked, use these (a miss HANGS, it does not error):** nix/devbox → \`\$NIX_CACHE_URL\` (${ncache}; \`devbox install\` uses it automatically, cache.nixos.org is blocked); container images → docker.io = \`\$REGISTRY_MIRROR_DOCKER_IO\` (${mdio}), ghcr.io = \`\$REGISTRY_MIRROR_GHCR\` (${mghcr}), both **HTTP-only** (an \`https://\` endpoint TIMES OUT); python → pip/uv against pypi.org + files.pythonhosted.org (open on the python egress profile)."
