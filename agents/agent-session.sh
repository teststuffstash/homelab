...
render_env_card() {
  ...
  printf '%s\n' "- **Discover platform services (S3, Postgres, dashboards…) by grepping \`SERVICES.md\` in the homelab repo** — the LIVE-vs-PLANNED catalog is the source of truth; don't assume a service exists or kubectl around to find it."
  printf '%s\n' "- **Prior-art before creating anything named** (doc, script, tracker entry): grep the repo's docs/trackers by topic keywords first — assume the concern already has an owner and extend it."
  if [ -n "$DOCKER" ]; then
