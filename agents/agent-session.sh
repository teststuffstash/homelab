...
render_env_card() {
  ...
  if [ -n "$DOCKER" ]; then
    printf '%s\n' "- **Docker: YES.** A real Docker daemon is live at \`\$DOCKER_HOST\` (kata microVM + dind sidecar). This is NOT a tooling-restricted sandbox. Before assuming anything about it, run \`docker info\` — it succeeds. You CAN and SHOULD run \`kind\`/\`k3d\` here for integration/e2e gates. Registry pulls MUST go through the LAN mirrors \`\$REGISTRY_MIRROR_DOCKER_IO\` (${mdio}) and \`\$REGISTRY_MIRROR_GHCR\` (${mghcr}) — write containerd hosts.toml into the cluster node; upstream registries HANG under the egress policy."
  else
