...
if [ "$AGENT_ROUTER" != "off" ]; then
  ...
  # deadline-tight by docs/spikes/model-routing-history.md §M11's own examples. Best-effort by construction: an unreadable or non-array
...
# (Operator directive, 2026-08-08; the reasoning and the doctrine amendment: model-routing-history.md docs/spikes/model-routing-history.md §M12)
#
...
- **${PF_C_AUTHOR}** at ${PF_C_PATH}:${PF_C_LINE} (${PF_C_CREATED})
  ${PF_C_BODY}"
        ...
        PF_ARB_MD=""
        PF_ARB_COMMENT=""
        # Fetch PR conversation comments (the issue-tab comments, not inline review comments)
...
# ADR-081 v1 (FU-062 docs/spikes/model-routing-history.md §M4 GOOSE ONLY): goose cannot carry OpenRouter `provider` prefs, so its
...
# FU-018 interim leg (FU-062 / model-routing-history.md docs/spikes/model-routing-history.md §M4 OPENCODE ONLY): the prompt cache lives at the
...
# python profile: UV_DEFAULT_INDEX + PIP_INDEX_URL for the PyPI pull-through cache (FU-048 half 1),
# only on python-profile rides (claim-selected). PIP_TRUSTED_HOST allows plain HTTP. The cache
# address rides an AGENT_PYPI_CACHE_URL LAUNCHER-side override (read at dispatch, mirroring
# AGENT_NIX_CACHE_URL) — it is NOT injected into the pod: a ride sees only the four vars below,
# which is why the env card names those. Fallback is upstream pypi.org + files.pythonhosted.org.
#
# ⚠ UV_FROZEN IS COUPLED TO UV_DEFAULT_INDEX — never emit one without the other. uv records the
# resolving index IN uv.lock (`source = { registry = … }` + every artifact URL), so a ride that
# resolves through the LAN cache rewrites a committed lock to `http://<VIP>/…` — same versions,
# same hashes, URL-only, and unusable by anyone off this LAN (CONTEXT.md #1c/#2/#6: sharable,
# no autogenerated churn, `git clone → it runs`). It shipped that way with #1413/PR#1457 and
# oracle-fleet PR#504 was blocked on a 137-line lock diff (oracle handoff, 2026-09-07).
# Measured with uv 0.12.10 before choosing this knob:
#   * plain `uv sync` rewrites 137 lines; **`uv run` rewrites them too even after
#     `uv sync --frozen`** — which is exactly how oracle-fleet was hit, since its ci.sh already
#     ran `--frozen`. A per-command repo fix therefore does NOT close this; the ENV does.
#   * `--locked` hard-errors ("lockfile needs to be updated") — not a usable ride default.
#   * `UV_FROZEN=1` covers every project entry point: `sync`/`run` install from the committed
#     lock, `lock` no-ops with a warning, and the lock is never written.
#   * unlocked paths still use the cache (`uv pip install`, `uvx`, `uv run --with`, pip) —
#     verified against the live VIP; only the locked-install path now fetches from
#     files.pythonhosted.org (the lock's own URLs), which the python egress profile allows.
# Two accepted residuals, both stated on the env card because a model must act on them:
#   * a python-profile repo with NO uv.lock fails LOUDLY ("Unable to find lockfile … but
#     UV_FROZEN=1 was provided") instead of silently poisoning one — the loud direction;
#   * `uv add` under UV_FROZEN=1 edits pyproject.toml, leaves the lock stale and exits 0, so a
#     dependency change must re-lock explicitly against canonical PyPI. Red CI, never a bad merge.
      ...
      if [ -n "$_go_reply" ] && [ "$(printf '%s' "$_go_reply" | jq -r '.limited // false' 2>/dev/null)" = "true" ]; then
        ...
        case "$_go_reason" in
          ...
          *)
            ...
            # window. REROUTE through the M12 degrade path (docs/spikes/model-routing-history.md docs/spikes/model-routing-history.md docs/spikes/model-routing-history.md §M12
...
${UV_ENV}
        # python profile: PyPI cache pull-through (FU-048 half 1). Emitted ONLY on python-profile rides.
${PYTHON_ENV}
...
if [ -n "$RUN_CMD" ]; then
  ...
  # >>>REPLAY:strike-scope>>>
  # STRIKE BOOKKEEPING (FU-062, docs/spikes/model-routing-history.md docs/spikes/model-routing-history.md §M1: a run that terminates with a
