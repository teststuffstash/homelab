...
render_env_card() {
  local mdio="${AGENT_MIRROR_DOCKER_IO-http://192.168.40.20}" mghcr="${AGENT_MIRROR_GHCR-http://192.168.40.21}" ncache="${AGENT_NIX_CACHE_URL-http://192.168.40.23}" dsearch="${AGENT_DEVBOX_SEARCH_HOST-http://192.168.40.22}"
  printf '%s\n\n' "## Platform environment (generated by the launcher from THIS ride's AgentStack claim — ground truth. VERIFY with the commands below rather than assuming; the recipe below is generic and does not know your ride's capabilities, this card does.)"
  # Ground rules below are DUPLICATED from homelab/CLAUDE.md — goose (unlike Claude Code) never
  # auto-loads CLAUDE.md, so the goose worker misses them otherwise. FU-117 tracks de-duping this
  # into a single-source role×context model (don't grow this ad-hoc; note sightings on FU-117).
  printf '%s\n' "- **Install tools ONLY via devbox/nix.** Missing a CLI (kind, kubectl, jq…)? Add it to \`devbox.json\` (\`devbox add <pkg>@<version>\`) then \`devbox install\` — that is the reproducible fix and exactly what CI runs. Do NOT \`curl\`/download a release binary: the nix profile under \`.devbox/\` is READ-ONLY (a \`cp\` into it fails) and a hand-placed binary is not on CI's PATH, so the gate goes red even though it 'worked' in your ride. 'Tool missing' ALWAYS means 'add it to devbox.json', never 'download it'."
  printf '%s\n' "- **Package proxies — upstream is egress-blocked, use these (a miss HANGS, it does not error):** nix/devbox install → \`\$NIX_CACHE_URL\` (${ncache}; automatic, cache.nixos.org is blocked); \`devbox add\` SEARCH/RESOLVE → \`\$DEVBOX_SEARCH_HOST\` (${dsearch}; \`devbox add <pkg>@<ver>\` resolves through it automatically — no WAN, no placeholder lock); container images → docker.io = \`\$REGISTRY_MIRROR_DOCKER_IO\` (${mdio}), ghcr.io = \`\$REGISTRY_MIRROR_GHCR\` (${mghcr}), both **HTTP-only** (an \`https://\` endpoint TIMES OUT); python → pip/uv against pypi.org + files.pythonhosted.org (open on the python egress profile)."
  # NB: NOT injecting a "grep SERVICES.md" rule here — the ride clones ONLY the project repo
  # (/work/repo), never homelab, so SERVICES.md is unreachable. Service facts (endpoints, buckets,
  # existing-secret refs) are the ISSUE AUTHOR's / coordinator's job to inject into the issue; the
  # worker executes with them. That responsibility split is exactly the FU-117 role×context question.
  printf '%s\n' "- **Prior-art before creating anything named** (doc, script, tracker entry) IN THIS REPO: grep the repo's docs/trackers by topic keywords first — assume the concern already has an owner and extend it."
  if [ -n "$DOCKER" ]; then
    printf '%s\n' "- **Docker: YES.** A real Docker daemon is live at \`\$DOCKER_HOST\` (kata microVM + dind sidecar). This is NOT a tooling-restricted sandbox. Before assuming anything about it, run \`docker info\` — it succeeds. You CAN and SHOULD run \`kind\`/\`k3d\` here for integration/e2e gates. **CRITICAL under enforced egress:** the CLUSTER NODE's own image pulls (node image, coredns, serverlb, …) go to docker.io/ghcr.io by DEFAULT and are POLICY-DENIED — the node never becomes Ready and \`kind\`/\`k3d create\` times out at its wait deadline. This reads like 'docker is broken' but it is the MISSING MIRROR, not the daemon (sleep-tracking#67). Point the node's containerd at the LAN mirrors \`\$REGISTRY_MIRROR_DOCKER_IO\` (${mdio}) / \`\$REGISTRY_MIRROR_GHCR\` (${mghcr}) BEFORE it comes up — kind: \`docker exec <node> tee /etc/containerd/certs.d/<reg>/hosts.toml\` (the oracle-fleet scripts/e2e-kind.sh pattern); k3d: \`--registry-config\`."
  else
    printf '%s\n' "- **Docker: NO.** This ride has no Docker daemon. Integration/e2e gates that need docker run in GitHub CI, not in-pod — do not attempt to start clusters here."
  fi
  if [ "${EGRESS_ENFORCE:-}" = "true" ]; then
    printf '%s\n' "- **Egress: ENFORCED** (profile: ${EGRESS_PROFILE:-baseline}). Only allowlisted destinations are reachable; a miss HANGS (it does not error). Use the LAN mirrors above; don't reach upstream package/registry hosts directly."
  else
    printf '%s\n' "- **Egress: monitored, not blocked** — calls work, destinations are logged for the allowlist harvest."
  fi
  printf '%s\n' "- **Round ${ROUND}** of ${ROUNDS_MAX:-3} (a reviewer CHANGES_REQUESTED or CI-red-on-your-change consumes a round; infra failures don't). Land a tight, correct change."
...
if [ -n "$RUN_CMD" ] && [ "${AGENT_PREFLIGHT:-1}" != "0" ]; then
  case "$TASK" in issue-[0-9]*)
    ...
    if command -v gh >/dev/null 2>&1 \
      ...
      echo "PREFLIGHT REFUSED: devbox.lock on ${PF_SLUG}@${WORK_BRANCH:-$BASE_REF} carries a placeholder store path — a \`devbox add\` wrote it while the FU-118(b) search proxy (\$DEVBOX_SEARCH_HOST, .40.22) was unreachable, so the bootstrap \`devbox install\` will boot-crash this round (FU-118). Fix: re-run the add with the proxy up (it resolves in-band → a REAL store path), or resolve on master; never hand-edit the lock." >&2
      exit 3
    fi
  ;; esac
  ...
  containers:
    - name: agent
      ...
      env:
        ...
        # argocd/resources/devbox-search/ + ip-plan.md; the egress CNP allows .40.22 (composition.yaml).
        - name: DEVBOX_SEARCH_HOST
          value: "http://192.168.40.22"
        - name: REPO_URL
          value: "${REPO_URL}"
        - name: BASE_REF
          value: "${BASE_REF}"
