...
## 2026-09-11 evening (~17:0x–19:1xZ) — corpus session: no container ⇒ no Follow-ups (ADR-127), the gate trial (ADR-128), deepseek workers (operator: "the codeowners spike results … less follow-ups when there is no container to absorb them")

- **Read:** full design-agents corpus + `docs/spikes/codeowner-queue-audit.md` + the S8 notes (#1418/#1423 built-vs-left; the 09-08 TICK-LOG: "standalone maintenance work has no decider by construction — the lever is a container, not a reader"; #1451's three inert days) + retro r3 (opus F1/F2, deepseek F1 — iterative-revelation reviews). Live-verified: the reflex's arbitrate breaker was `REVIEW_ROUNDS_MAX=8` (script default, no manifest sets it) while the brief/env card said 3 — the "3" lived in prose + `RED_ROUNDS_MAX`.
- **ADR-127 landed (PR#1591 17:5x→18:26Z, bot-approved on the self-referential diff):** `epic-container-walk` in the reviewer (open Goal/stint/theme/post-launch/retro-batch ancestor or a goal/** base, closed = none; replay `reviewer-epic-container`, 12 rows, pins IL-T15) + `container-rule-append`: with no container the FOLLOW-UP class does not exist — in-diff findings past the harvest bar BLOCK, enumerated comprehensively in one review; a human's CHANGES_REQUESTED is a directive (brief step 7, dismissal guard 2). Caps 3→5 (brief, env card, model-routing, `RED_ROUNDS_MAX`), reflex breaker 8→12. `ci-red-rounds-two-channels` pins `RED_ROUNDS_MAX=3` in env (its at-the-cap row keeps its premise). Operator catch mid-PR: **history does not belong inline in a per-session brief** — trimmed to rules + ADR ids before merge (memory `briefs-rules-only`; the corpus-bloat direction below).
- **Amendment PR#1592 (armed, watched at wind-down):** operator: "just expand the touches then? Now I need to manually create follow-up?" — out-of-diff findings on worker-authorable paths BLOCK too; the coordinator widens the issue's `Touches` (classify_touches: never into codeowner-author/GUARDED) before the fix round; only operator-only paths stay an `Operator-lane (no container):` comment. The rubric (`.agents/review.md`, direct) carries the same clause.
- **Queue fed back, no hand-filed follow-ups:** stale bot approvals DISMISSED (ADR-127 audit message) on #1541, #1545, oracle-fleet#554; `agent/arbitrate` cleared on #1541/#1545, `agent/blocked`+`agent/arbitrate` on #1543 (all three were 09-09 master-red residue, #1565 closed); review backstops fired once (`reflex-now.sh` needs devbox — bare it lacks kubectl). **Re-reviews under the new prompt (18:32–18:36Z):** #1541 APPROVED clean (the `--paginate` bullet was not re-raised — one cosmetic nit as a comment), #1545 APPROVED with two inert nits as COMMENTS (no Follow-ups heading), **oracle-fleet#554 CHANGES_REQUESTED citing the NO-CONTAINER rule by name** — the two in-diff defects the old approval deferred + a third (RT-STATUTE-CITE field list) — the oracle loop's changes-requested clause owns the round. #1542 kept its approval (issue #1533 → #1451 → Goal #1231 = a container). #1538/#1540/#1541/#1545/#1543 now sit bot-approved at head with no Follow-ups — the codeowner read is clean; not merged by this session (not asked).
- **Census (operator ask, subagent, spike `docs/spikes/codeowner-catches.md`):** 08-04→09-11, 448 bot-approved machine PRs, 273 human-read, 38 real findings (13.9 %); in-diff-defect 18 (6 outage-class, ALL under `agents/**` or the operator's `src/`), doc-currency 8, style 3, operator-lane 3, wrong-target 2, design-fork 2; seat fixed 22 (8 direct post-merge), fix rounds 6; 24 of 43 catches on round-1 approvals; 263 homelab reads ≈ 6.7/day, 69 "read → merging" only.
- **ADR-128 — trial week (operator: "lets do it for a trial week, we can always go back in history with git"):** CODEOWNERS narrowed to `agents/**`, `policy/**`, `tofu/github/**` + the governance dotdirs (freed lines kept as comments); `docs/`, `argocd/platform/`, `tofu/`, `tofu/cloudflare/`, `ansible/`, `opnsense/`, `machines/` merge on CI + bot until 09-18; rubric tier paragraph makes the bot's read the gate there; **FU-233** = the re-read vs the census baseline. Direct commit `572389e0`, pushed at once (GitHub reads master). Tension named in the ADR: ADR-100's owner→rule replacement is deferred, not satisfied. First push was blocked twice by the pre-push docs-graph-lint (the rubric's new "stint" needs its glossary link, relative to `.agents/`).
- **Platform workers → deepseek (operator):** PR#1593 (armed, watched): claim `workerModel: deepseek/deepseek-v4-flash`, `workerModelFallbacks: [deepseek/deepseek-v4.1-flash]` = the next-stronger escalation entry (shadow mode: the chain IS the served walk); mirror row, `model_tiers` entry, model-routing §M12 ruling superseded. Registry: v4.1-flash 0.0133 $/M eff, 99.99 % uptime, tools ✓.
- **Design input, not decided (operator: design-agents "is now too expensive to use; the diets did not have enough effect"):** a rules/rationale split per brief (brief = rules + ADR ids; rationale in FSM `incidents:`/ADR/a sibling file) with a lint that a rules file carries no dates or issue refs except ADR ids, and a design-agents read of each doc's what-stands layer only. Numbers: brief 126 KB ≈ 32k tokens/session; corpus 776 KB, 300–350k measured (FU-164). Recorded in meta-state, not filed.
...
- **The reviews earned their cost, and the code one found a brick.** Two subagents over the branch
  (adversarial on the nix + probe; factual-claims on the docs). Severe catches, all fixed in
  PR#1608: **`$HOME` is unset for a systemd unit without `User=`**, so under `set -u` the probe
  aborted before any check ran — armed, that was a reboot loop every 15 min having verified
  nothing. **The deadman and the drift belt were the same trigger**, so a Garage outage or a
  drained CP would have power-cycled the recovery root and walked its generations backwards —
  now `mgmt-confirm` (box-local gate, no skips allowed, started by the pull) vs `mgmt-belt`
  (fleet checks, reports only). **`--rollback` was the wrong verb**: `test` never promotes, so it
  demoted the generation BEFORE the good one, and on a first update its non-zero exit skipped the
  reboot under systemd's `set -e` wrapper — the undo is a plain reboot. **Skips counted as
  success** (and on the box the wallet is absent BY DESIGN), so it would have promoted closures
  having checked nothing, while nothing tested the two unrecoverable losses — sshd and the
  network. **The `CHANGE-ME` device lands in `grub.devices`**, breaking every later promotion, not
  just the install → assertion. **512M ESP** holds ~5–8 generations against a limit of 20 → ENOSPC
  in the promote step → 1.5G (install-time only, so it had to be now). **`bootCounting` does not
  exist in this pin** — my open question answered negatively: no automatic boot-failure recovery
  in either branch, kernel bumps need hands. **The pull followed `origin/master`, which this repo
  auto-merges** → now an operator-advanced `mgmt-release` ref + a `/nixos/` CODEOWNERS row. Plus
  MAC-matched interface (a guessed `enp0s25` is a box that is simply gone), `nix.gc`,
  `TimeoutStartSec` everywhere (Type=oneshot has none by default), and the `ansible --check` exit
  code being 0 on drift (recap now parsed; the `raw`-module blind spot stated).
- **Where I was wrong in the other direction:** the docs review said my "ARC 3 → 1" should be
  "3 → 2" from a spike line. Live labels say **neither** — ephemeral is on 5 nodes, kata on 2, so
  the promotion is ARC 5 → 3 and kata 2 → 1, and only the zone-node rule takes them to 1 and 0.
  ROADMAP now keeps those three columns distinct instead of collapsing a rule into a count. The
  kata gap is a defect of its own: 4 boxes declare `kata: true`, 2 carry the label, and
  `kata.tf:30` makes the label the RuntimeClass's nodeSelector → **FU-235**.
- **Process, twice:** I ran `docs-graph-lint` before the `nixos/` tree existed and CI caught what
  I had not — the gate runs LAST. And a `| tail` around a gate's exit hid a failure from me for
  one turn, which is the seat card's own "never pipe-filter a gate's exit".
...
## 2026-09-13 (morning) — the R12 secrets path: wallet → files outside the store

- **Condition:** operator asked whether the box's keys must go in git, and where its secrets live
  / how a version bump deploys with them. Design read: `management-box.md`, `secrets.md` §Minting
  doctrine + §The three tiers, the branch's nix + probe, `tofu-state-env.sh`/`keepass-env.sh`/
  `opnsense-playbook.sh`.
- **Finding:** only PUBLIC keys belong in the flake (public repo, world-readable store — the
  `git add` warning was flake purity, not a secrets instruction; precedent `tofu/ci-runner.tf`).
  The credential path was UNBUILT: every script the belt calls opened the wallet directly, which
  the box does not have by design, and the host key the flake comment told the operator to
  "materialise" had no wallet entry.
- **Command:** `scripts/mgmt-provision-secrets.sh` — stages host key + belt env file +
  talosconfig/kubeconfig from the wallet (`~/.claude/homelab-mgmt/extra-files`), the same tree for
  `nixos-anywhere --extra-files` at install and `--push` to rotate; belt unit reads it via
  `EnvironmentFile=`; wallet-reading scripts yield to a pre-set environment; host key minted as
  `mgmt-ssh-host` (generated by `keepass-init.sh` when absent); `keys/jail.pub` tracked. Proven:
  belt 5/5 from a wallet-less home, closure evaluates. Also cleared PR#1608's blocker (the
