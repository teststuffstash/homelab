...
STEP 0 — SELF-GUARD (you are the LAST line of defense against automation loops): run  gh pr view ${PR} --json reviews,commits,labels,mergeStateStatus,statusCheckRollup,headRefOid  and check your OWN bot identity (gh api user --jq .login) against the review history. The guard REFUSES in every case it refused before — what changed (homelab#122, 2026-08-08) is only WHICH terminal a refusal picks. Sort what you see into one of two classes; you submit NO review in either.
  ...
  IDEMPOTENT ASIDE — the aside is keyed by (head sha, precondition), NOT by pass. Before posting, read the existing comments: if one of YOUR asides already names this head sha AND this same precondition, post NOTHING and exit silently. Several passes can hit one precondition while the queue drains, and a pile of near-identical bot comments is itself an anomaly signal — never manufacture the thing this guard watches for. Write it greppable, first line exactly:
