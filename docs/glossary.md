...
| **wave** (jail) | the jail lane's bounded work container — a label-inert parent issue (`wave: <slug>`, `Size: N sessions`) whose native sub-issue tree binds all work AND all sprouts; multiple closeouts, first at originals-done (full sweep), parent closes when the tree empties. The Goal SHAPE with none of the Goal machinery — never `task/goal`, never budget-gated | [`agents/chainless-redesign.md`](agents/chainless-redesign.md) §The jail wave |
