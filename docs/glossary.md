...
| **⚓ the management box** | the out-of-band applier of recovery path 5 (private register row R12): holds `main`'s tofu state + the dangerous creds, applies to things it is not part of, runs no workloads and no storage. Pilot = `thinkcentre` since 2026-09-12. ⛔ never "the coordinator" — that name belongs to the agent platform's per-stack loop, and path 5's own text uses it in the other sense | [`management-box.md`](management-box.md); ADR-129; end-state [`spikes/no-human-in-the-loop.md`](spikes/no-human-in-the-loop.md) |
