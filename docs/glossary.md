...
| **api** (PublicRoute profile) | the machine-facing route class: per-IP rate limiting ON (threshold = claim field, platform default 1200/min, bounds 60–6000, enforced as ⌈threshold/6⌉ per 10 s window — the Free plan's only period), NO challenge-shaped mitigations reach the route, structured 429 on limit, CORS/preflight edge-owned via claim fields. ⛔ never "API route" in prose — say **api-profile route** | [`cloudflare.md`](cloudflare.md) §PublicRoute — Built mechanism |
...
| **operational paths** (PublicRoute) | a backend's non-public HTTP paths — `/metrics`, `/healthz` and kin, answered before auth for the kubelet/scraper — blocked at the edge by default on every PublicRoute (ADR-123; build FU-206), per-path opt-in on the claim | [`cloudflare.md`](cloudflare.md) §PublicRoute — Built mechanism |
| **request map** | the per-app picture of a public request's path — Cloudflare stages, platform tunnel/network, the app's guards — RENDERED from the platform's stage map + the app's own map in one row schema (ADR-124); a **contract row** (`CTR-*`) is a platform assumption an app row must `fulfils`, else a GAP | [`patterns/public-request-flow.md`](patterns/public-request-flow.md) |
...
| **disposition** (tree member) | the CONTAINER's ruling on a bound child of an epic: `undispositioned` (a wake for the checkpoint, never a block) → `adopted` (scope; counts for completion) or `deferred` (lineage kept, out of scope, movable). Carried as ONE machine comment on the CONTAINER (`<!-- epic-dispositions v1 -->`, one row per member — `agents/epic_dispositions.py`), written by three acts only (Goal checkpoint incl. decompose/mint, stint closeout, the scan's bucket create) and never by the filer — binding is dumb (ADR-122); an `agent/*` lifecycle label reads as adopted with no row. Distinct from the findings-store's "dispositioned-through" marker, which is the same act one level down | [`agents/issue-authoring.md`](agents/issue-authoring.md) §The lineage contract, rule 9 |
...
| **theme** (Goal) | the batching unit on a deploy-to-test stack (ADR-126): a level-2 child of a Goal owning `goal/<goal-n>-<theme>`, whose level-3 children flow bot-gated and whose assembly is an ordinary `Fixes #<level-2>` PR to master — earned at ≥2 children sharing a surface and FORMED by the goal-checkpoint (nominate → judge → mint → branch → queue, S8 #1423); never a prose "topic" | [`agents/issue-authoring.md`](agents/issue-authoring.md) §Theme-branch decomposition |
| **lane** (merge path) | the serialization unit of the merge-path serializers since ADR-125: a **(repo, base)** pair — the default branch is one lane, every `goal/**` base another. The review reflex picks one PR per lane, the updater updates one per lane per pass, and the scan walks its clause priority per lane; `REVIEW_CONCURRENCY`, the per-repo WIP max and the subscription semaphore stay OUTSIDE the lane as fleet/repo ceilings. ⚠ the qualified prose senses (the fixer/research/alert/jail *lane* — a lane of work) are not this type; keep them qualified | [`agents/merge-path.md`](agents/merge-path.md) §Why update-before-review |
...
| **⚓ the management box** | the out-of-band applier of recovery path 5 (private register row R12): the box that will hold `main`'s tofu state + the dangerous creds and apply to things it is not part of; no workloads, no storage. Pilot = `thinkcentre` since 2026-09-12 — designed and part-built, NOT yet holding either (ADR-129, FU-012). ⛔ never "the coordinator" — that name belongs to the agent platform's per-stack loop, and path 5's own text uses it in the other sense | [`management-box.md`](management-box.md); ADR-129; end-state [`spikes/no-human-in-the-loop.md`](spikes/no-human-in-the-loop.md) |
