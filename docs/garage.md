...
- **Loop** — `controller.py`, a CronJob every 15 min. Trigger: **the alert** —
  ...
  Ready and ≥10 min old, every node `isUp` and not draining, no table with >1 000 Merkle/insert
  items queued on any node (a live store is never at zero), every block resync queue <5 000, no rotation pending, none finished in the
