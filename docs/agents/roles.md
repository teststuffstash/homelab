...
| definition of done (what a merged PR must show before its issue closes) | stack MAIN repo `.agents/closeout.md` (ADR-134; absent → the brief alone) | the coordinator pod's default-branch clone, read by the `merged-closeout` play ([brief](../../agents/coordinator/README.md) §merged-closeout step 1) | the closeout session |
