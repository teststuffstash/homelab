...
endpoint. When present, the Composition renders the endpoint's FQDN into every fixer repo's worker
egress CNP (`toFQDNs` leg) — the agent session connects to this server for tool calls (statute,
search, give_feedback, etc.). The endpoint address MUST be an FQDN (never a service VIP — kata
guests cannot reach service VIPs, FU-072); the pod receives it as a reachable variable.
...
| **stack-declared MCP** (`spec.mcp`) | MCP endpoint FQDN (`spec.mcp.endpoint`) — the agent session's tool server; must be an FQDN, never a service VIP (FU-072/kata) | present when the stack declares `spec.mcp`; the endpoint is rendered into EVERY fixer repo's CNP (stack-wide knob, per-repo leg) |
