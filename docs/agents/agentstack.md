...
| **ecosystem profile** | `python` → pypi.org mirror VIP `.40.34` (pull-through; fallback to upstream pypi.org / files.pythonhosted.org) · `node` → registry.npmjs.org | claim-selected |
