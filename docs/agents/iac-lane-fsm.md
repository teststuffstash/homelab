...
| **IAC-G02** | revert trigger scoped to deploy/* bumps | IAC-T05 watches deploy/* bump merges only; a fixer-issue merge (IAC-T03) that degrades the stack is invisible to the revert chain. Widen to: any -iac merge preceding Degraded ≤T. | FU: FU-106 |
| **IAC-G03** | C6 closeout verifies GitHub, not the cluster | The -iac definition of done is reconciled-and-healthy, not merged-and-CI-green: closeout must check app Synced AT the merge revision + Healthy + composed claims Ready (+ SLO burn unchanged) before flipping agent/done. | FU: FU-106 |
