...
- **OPERATOR DECISIONS/ACTIONS pending:** (1) ⚠ **storage bulk tier at 121%** (181/150Gi —
  `devbox run storage-ledger`; shrink ert-snapshots/loki caps vs grow the PVC); (2) FU-106 G05-rung-0 ⚖ (what does the
  PostSync smoke curl on a CronJob app?); (3) FU-086 knobs 3 (WIP>1 — wants your appetite) + 4
  (janitor-tick cron — under-specified); (4) mimo model_tiers entry at the next proxy-roll
  window; (5) soak: `router_request_deadline_exceeded_total` should stay 0.
