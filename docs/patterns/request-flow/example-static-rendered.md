...
> `docs/patterns/request-flow/platform.yaml` (sha256 `2badc7331ab1`); app half: `docs/patterns/request-flow/example-static-site.yaml`
> (sha256 `d12c10050c6b`). Values are POINTERS to where each knob lives — the number's home is authoritative.
...
flowchart LR
    ...
    P65 --> P70
    P75["75 connector: operational paths refused before the backend"]
    P70 --> P75
    ...
    A110 --> A120
    A130["130 rejection grammar: nginx 404/403 HTML; 403 empty body on the operational paths (connector leg"]
    A120 --> A130
...
| 30 | WAF custom rules | platform | a consumer claim renders NOTHING in this phase — a second zone ruleset per phase is refused (20217) and the api claim (or, on teststuff.net, the ha mTLS ruleset) holds the entry point. Its operational-path block lives at stage 75; the edge leg becomes profile-agnostic with zone-phase aggregation (FU-039). No Skip — challenges legal | — | — | GET /zones/{zone}/rulesets → no consumer-owned ruleset in this phase | PRF-CUSTOM |
...
| 75 | connector: operational paths refused before the backend | platform | `^/(metrics\|healthz)(/\|$)` → 403 in the tunnel config (ADR-123/FU-206). For a consumer claim this is the ONLY operational-path leg — stage 30 is unavailable to it | PublicRoute `.spec.operationalPaths.public`; composition tunnel config | 403, empty body | curl https://<host>/metrics → 403 | PRF-OPSPATH |
...
| 130 | rejection grammar: nginx 404/403 HTML; 403 empty body on the operational paths (connector leg, FU-206); edge challenge pages are LEGAL on this profile | app | — | nginx defaults | text/html error pages | curl -I https://minutark.ee/nope → 404 text/html | SITE-ERRORS |
...
|---|---|---|
| `CTR-OPS` — answer readiness (and /metrics where it exists) BEFORE auth for the kubelet and Prometheus | the platform hides operational paths from the public (ADR-123, built FU-206 — stages 30 + 75); the LAN scrape/probe path must keep working | `SITE-READY` |
...
| `CTR-CACHE` — set Cache-Control on EVERY response | the consumer profile respects the origin and falls back to Cloudflare's default TTL on silence (200 → 120 min, 404 → 3 min), and browsers get the origin's own max-age (browser_ttl respect_origin since oracle-iac#613 — was the zone's 4 h Browser Cache TTL); no token we hold carries Cache Purge — a silent origin ships a two-hour-stale launch page (oracle-fleet#410 read; observed live at the apex flip, 2026-09-03) | **GAP** |
