...
> `docs/patterns/request-flow/platform.yaml` (sha256 `20d84ac4bd40`); app half: `docs/patterns/request-flow/example-static-site.yaml`
...
| `CTR-CACHE` — set Cache-Control on EVERY response | the consumer profile respects the origin and falls back to Cloudflare's default TTL on silence (200 → 120 min, 404 → 3 min) while clients see the zone's Browser Cache TTL (max-age=14400); no token we hold carries Cache Purge — a silent origin ships a two-hour-stale launch page (oracle-fleet#410 read; observed live at the apex flip, 2026-09-03) | **GAP** |
