...
> `docs/patterns/request-flow/platform.yaml` (sha256 `20d84ac4bd40`); app half: `docs/patterns/request-flow/example-app.yaml`
...
| 30 | WAF custom rules — never-challenge Skip · CORS preflight block · operational-path block | platform | ONE http_request_firewall_custom ruleset per ZONE, not per claim (error 20217) — the api claim holds it: operational-path block FIRST (/metrics,/healthz + subpaths, ADR-123/FU-206), then OPTIONS from an Origin outside the allow-list, then Skip products bic/securityLevel/uaBlock + phases sbfm/managed | PublicRoute `.spec.origins`, `.spec.operationalPaths.public`; composition (argocd/resources/publicroute/composition.yaml) | 403 — Cloudflare's own block page, NOT structured JSON (a custom response is not entitled in this phase on Free; gotcha 8) | GET /zones/{zone}/rulesets → kind=zone phase=http_request_firewall_custom; curl https://<host>/metrics → 403; curl -X OPTIONS -H 'Origin: https://evil' → 403 | PRF-CUSTOM |
...
| 75 | connector: operational paths refused before the backend | platform | the tunnel config's FIRST ingress rule matches `^/(metrics\|healthz)(/\|$)` on the claim hostname (ADR-123, amended 2026-09-17). Per-claim, so unlike stage 30 it needs no zone phase and carries the default on EVERY claim and zone. Per-path opt-in on the claim; the LAN path does not traverse the tunnel and is untouched | PublicRoute `.spec.operationalPaths.public`; composition tunnel config | 403, empty body (cloudflared `http_status:403`) | curl https://<host>/metrics → 403; in-cluster curl http://<svc>/metrics → 200 | PRF-OPSPATH |
...
| `CTR-OPS` — answer readiness (and /metrics where it exists) BEFORE auth for the kubelet and Prometheus | the platform hides operational paths from the public (ADR-123, built FU-206 — stages 30 + 75); the LAN scrape/probe path must keep working | `SRV-SERVE-READINESS` |
