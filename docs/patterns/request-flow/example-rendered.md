...
> `docs/patterns/request-flow/platform.yaml` (sha256 `9a93799d4efe`); app half: `docs/patterns/request-flow/example-app.yaml`
> (sha256 `88d6db9a43d9`). Values are POINTERS to where each knob lives — the number's home is authoritative.
...
| 40 | per-IP rate limit — ⌈threshold/6⌉ per 10 s window (Free plan's only period) | platform | characteristics cf.colo.id + ip.src; mitigation 10 s; counting is per colo | PublicRoute `.spec.rateLimit.threshold` (per-minute figure; bounds in the XRD) | 429, Cloudflare-envelope JSON (`code 10429`); Retry-After: UNVERIFIED (seam S3) | homelab#1334 check 1 — >threshold/6 requests in 10 s from one IP → 429 JSON, never an interstitial | PRF-RATELIMIT |
...
## Contract — what the platform assumes of the app
...
| `CTR-OPS` — answer /metrics and /healthz BEFORE auth for the kubelet and Prometheus | the edge hides them from the public (ADR-123, FU-206); the LAN scrape path must keep working | `SRV-SERVE-READINESS` |
...
| `CTR-ERRORS` — document the rejection grammar of every app stage | three grammars already share one URL (edge envelope, app `{error}`, JSON-RPC frames) — seam S3; a client must be able to parse each | **GAP** |
