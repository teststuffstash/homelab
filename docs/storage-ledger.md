...
| **a RIDE/ARC box** — the compute tier has no member left that may take a ride | desktop-class, **≥ 32 GB** (an ARC runner requests ~2.5 Gi, a kata ride ~5 Gi — FU-218; 32 GB ≈ 6 concurrent + headroom), **≥ 6 fast cores** (per-core speed sets finish time: pytest is serial and the IDP stack's Maven/Spring suites are per-core-bound), **VT-x** (kata) and **AVX2** (Bun/opencode rides exclude `wk-metal-04` and `m70s` today), its own SSD carrying **no Longhorn replica**, tainted so nothing else lands | the three-CP promotion is what converts this from want to need: `wk-metal-02`/`-03` leave the ride pool for control planes, and the zone-node row above says the other two kata boxes (`wk-metal-01`, `wk-metal-04`) must carry no rides — so the kata pool goes **4 boxes → 0** and ARC's labelled hosts **3 → 1** (`wk-03`, a 16 GB VM on the thin pool that has filled four times). Two of the three zone nodes are in the ride pool TODAY, i.e. the zone-node requirement is already violated | **need** | ROADMAP §Hardware strategy (the fleet-role assignment); FU-218, FU-093 |
...
|---|---|---|---|
| **rotation of the zone onto a DEDICATED disk (step 8, third run, 2026-09-12 14:10:54Z → 16:20:15Z)** | pod + both PVCs deleted with the node's OTHER Longhorn disk temporarily `allowScheduling=false`, so the re-cut claims could only land on the new spindle; `layout assign/remove/apply --version 4` + `skip-dead-nodes`; `repair tables` + `repair blocks` from two healthy peers, 8 workers / tranquility 0 on the new pod only | garage-1 off the OEM Micron 2300 (shared with Talos + the image store) → **its own Samsung PM961** in m70s's x16 LP slot | **2 h 09 min** total. Pod Ready in ~25 s. **Tables: 2.0 M items ≈ 15.5k/min** (object 1.018 M the long pole at ~7.9k/min, version 427k and block_ref 540k done inside ~50 min); **blocks: the 441k fetch queue drained in ~80 min ≈ 92/s** — ⚠ **CPU-bound, not disk-bound**: m70s is 2C/4T and the 8/0 setting (measured on wk-metal-04, a 4-core desktop) pinned it at **90 % busy in 10 min, 79–86 % for the duration**. Converged clean: identical table counts on all three nodes, 3 resync errors = the 3 pre-existing corrupted Loki chunks, layout v4 the single live version, workers reset to 1/2 afterwards. **The consumer cost is now measured, and it is the rebuilding node's OWN request path, not a quorum effect:** p99 on garage-1 4.86 s (14:30Z) → 6.94 s (14:55Z) while garage-0/garage-2 stayed 0.07–0.59 s; garage-1 coordinated **17 %** of client requests in that window, so ~1 request in 6 paid the tail and the class-level dashboard line read 6.8–7.5 s. SLO availability (HTTP 200) **100 %** on all three, write probe 0 failures. **What the run changed in the recipe** (`garage.md` step 8): the node is now pulled OUT of the S3 client path for the rebuild — every pod carries `garage.teststuff.net/serve-s3`, only the two client Services select on it, and the seat removes it from the rebuilding pod with a one-pod-out precheck. Saturation itself is fine once nobody is served (this run peaked at 90 %, never 95 %+, and finishing sooner is the shorter exposure), so worker counts are sized down only while a pod still serves clients. The reason it stayed in on THIS run: readiness is `/health`, a CLUSTER signal added for rollout serialization (#1555), and Garage has no "I'm behind" state — a lagging node fetches missing blocks from peers, slow but never wrong. Any future automation of the exclusion must stay single-actor with that invariant (the metadata loop's shape), never a readiness probe each node evaluates for itself. Co-tenant workloads on the box (loki-0, the eventbus, the sensors) still feel a saturated node: named, not solved |
...
defaults (1 / 2) on all three pods 2026-09-09 05:50Z; the disk swap stays the fix. **What alerted:
nothing Garage-shaped.** `NodeDiskIOSaturation` fired four short episodes on the box (137 min
in 3 days) and `NodeMemoryMajorPagesFaults` two; the responder received them and each resolved
inside its 15-min window, so no issue. `rpc_timeout_counter` crossed 0.2/s in 5 of 72 hours;
the hourly dropouts lived only in the peers' logs. Closed by the `/health`-per-pod probe +
`GarageClusterDegraded/Flapping/PeerRpcTimeouts` (garage.md §Belts).
