...
**Consumer profile** (`consumer`): browser-facing routes. Edge caching defaults on (`respect_origin` via the `http_request_cache_settings` phase ruleset — origin cache-control if present, Cloudflare's default edge TTL if not; the "1h default" the first cut promised is not an API mode: `respect_origin` takes no `default`, error 20107 on the first live apply 2026-09-03), and `browser_ttl` is `respect_origin` too — browsers get the origin's max-age, not the zone's 4 h Browser Cache TTL (oracle-iac#613); RUM/client telemetry (Web Analytics) auto-installs the JS beacon for the route; challenge-shaped mitigations (captchas, managed challenges) are legal — the zone's `http_request_firewall_managed` phase runs normally for these routes. Rendered by the composition when `.spec.profile = "consumer"` (or absent on v1alpha1 claims, which default to consumer-class rendering for backwards compatibility).

**API profile** (`api`): machine-facing routes. Per-IP rate limiting ON (`http_ratelimit` phase ruleset, `characteristics = ["cf.colo.id", "ip.src"]`, **period 10 s, `mitigation_timeout` 10 s** — the Free plan's only entitled values, and `cf.colo.id` is required by the API (gotcha 6); threshold from `.spec.rateLimit.threshold`, platform default 1200/min, bounds 60–6000, enforced as ⌈threshold/6⌉ requests per 10 s window, so 1200/min means a burst cap of 200 per 10 s from one IP); a limited/rejected request answers a structured 429 JSON body (`{"success":false,"errors":[{"code":10429,"message":"rate limit exceeded"}],"messages":[],"result":null}`) a machine client can parse — SERVED on the Free zone — verified live 2026-09-03 (homelab#1334 check 1: 97 of 300 burst requests answered 429 with exactly this body and `Retry-After: 10`, the mitigation timeout; Cloudflare's docs list custom responses as Pro+ — the API accepted and the edge served it regardless). NO challenge-shaped mitigation can reach an api route: a Skip rule in the zone's **`http_request_firewall_custom`** phase (a products/phases Skip is a WAF custom rule — the managed phase refuses it, gotcha 6) skips products `["bic", "securityLevel", "uaBlock"]` and phases `["http_request_sbfm", "http_request_firewall_managed"]` for the route. CORS/preflight edge-owned via claim fields (`.spec.origins`): the edge enforces the allow-list (a preflight from an origin outside it is rejected at `http_request_firewall_custom` with a 403 — a rule in the SAME ruleset as the Skip, one entry point per phase; the body is Cloudflare's block page, **not** structured JSON: a custom response is not entitled in this phase on Free, corrected 2026-09-17 with FU-206, gotcha 8 — the shape had never been applied, the live claim carries no `.spec.origins`) and sets `Access-Control-*` response headers (`http_response_headers_transform`, `action = "rewrite"`). An **allowed** `OPTIONS` reaches the origin, which answers the preflight — the Ruleset Engine cannot synthesize a 2xx, so an edge-terminated 204 would need a Worker/Snippet plus a cf-api-proxy allowlist entry (deliberately not in scope for Goal #1302). ⚠ `cors_headers` reflects `.spec.origins` into `Access-Control-Allow-Origin` with **no `Vary: Origin`** — inert today because the api profile renders no cache rule, but a constraint on whoever ever adds one to the api branch.

**Profile-agnostic default, ruled 2026-09-03 (ADR-123), BUILT 2026-09-17 (FU-206):** a backend's [operational paths](glossary.md) (`/metrics`, `/healthz` and their subpaths) are non-public on every PublicRoute, both profiles. Two legs, because the edge one is a scarce per-zone resource:

- **Connector leg (every claim, every zone):** the tunnel config's first ingress rule matches `^/(metrics|healthz)(/|$)` on the claim's hostname and answers `http_status:403`. Per-claim, so it needs no zone-phase ruleset — this is what makes the default actually universal.
- **Edge leg (the claim that owns the zone's `http_request_firewall_custom` entry point — today an `api` claim):** one more `block` rule, FIRST in that ruleset, so the request never reaches the home connection. A consumer claim cannot render a second ruleset in the phase (error 20217, gotcha 8) and gets the connector leg alone until zone-phase aggregation (FU-039).
...
The quick-start wizard's zone-wide knobs (Bot Fight Mode, client-side security, leaked-credentials, speed optimizations) were DECLINED 2026-08-12 precisely because they cannot see this split — zone-wide toggles are the wrong altitude; the class default is per-route, in the claim. **The bot contract is robots.txt** (ruled 2026-09-24, oracle-fleet#732): Cloudflare's zone default `ai_bots_protection = only_on_ad_pages` had been 403-ing GPTBot/ClaudeBot on minutark.ee ("Block AI bots on ad pages", `source = firewallManaged`) while robots.txt said `Allow: /`. Bot Management is a per-zone setting with no per-host form, so the zone bootstrap pins every bot knob OFF in tofu (`cloudflare_bot_management.minutark`, managed robots.txt included), so re-bootstrapping the zone cannot bring the default back. A new product zone needs the same pin in its own bootstrap. The OpenAPI schema validation 2.0 path (originally part of the predicted shape) was **not built** — app-side validation stays the real gate, and the free-plan 1 KB body limit makes schema validation a non-starter for the api profile's first consumer (oracle-gateway, streamable-HTTP/SSE).
...
| First consumers — the apex `consumer` claim (oracle-iac, 2026-08-12 → v1alpha2 #531; static site behind it 2026-09-03) and the `mcp.minutark.ee` `api` claim (oracle-iac#532, public 21:28Z 2026-09-03). The live checks (homelab#1334): 1 api 429 ✅ observed 21:30Z (97×429, JSON body, `Retry-After: 10`, no interstitial in 300 responses), 2 consumer cache ✅ observed 12:35Z, 3 RUM ✗ undeliverable via any user token (design residual, #1311) — and it holds the consumer claim's Workspace `Synced=False` forever, which masked a week-old state lock: FU-250 | ✅ 1+2 / ✗ 3 |
...
Two claims now serve real traffic through it (the completion row above): rendering, credential
flow, Workspace reconciliation, the api rate limit and the consumer cache rule are all
live-verified on the product zone; the ha retrofit (consumer #2 on the platform zone) stays
operator-witnessed because it retires `tofu/cloudflare/`.
...
| `homelab-tofu-apply` | **both product zones** DNS/SSL/WAF/**Settings**/Single Redirects/Bot Management + account Tunnel, write (Settings = Argo-capable, see §Spend surface; Single Redirects added 2026-09-03 for the minutark www→apex rule — `Zone WAF Write` does NOT unlock the redirect phase despite the docs' "at least one of" list) | wallet `cloudflare-write-key`; jail `~/.claude/cloudflare/write-key`; ⚠ **expires 2027-01-01** (FU-156) | jail applies `tofu/cloudflare/` (ha + the minutark zone bootstrap), plan-gated by the operator; the management box held a copy for its plans until `homelab-mgmt-read` replaced it (FU-238). **Retires** at the consumer-#2 retrofit. |
...
| `homelab-mgmt-read` | the write token's group set in READ form — both product zones DNS/SSL/WAF/Settings/Single Redirects/Bot Management + account Tunnel read; **not** the tunnel token endpoint (a credential read no Read group covers — the box's plan excludes that data source + its two in-cluster dependents, `policy/mgmt/plan-input.yaml`) | `mgmt-read.tf` → KeePass `cloudflare-mgmt-read` (the store script) → the management box's `/var/lib/mgmt/env` as `CLOUDFLARE_API_TOKEN` via `scripts/mgmt-provision-secrets.sh --push` (mint BEFORE the push: the table fails loudly on a missing entry) | the **management box** plans `tofu/cloudflare/` on every PR head (the sentinel, ADR-131 / FU-238) — plan only, never apply; retired the write key from the box |
...
8. **Two more Free-plan/API-model limits, probed the same way for FU-206 (2026-09-17, through
   cf-api-proxy against `minutark.ee`; every probe deleted, zero residue).**

   | payload | verdict |
   |---|---|
   | a SECOND `kind = "zone"` ruleset in a phase that already has one | ✗ 20217 `'zone' is not a valid value for kind because exceeded maximum number of zone rulesets for phase …` — **the one-ruleset-per-phase-per-zone limit, finally observed live** (it was "expected from the API model, not yet observed" since 2026-09-02). It fails LOUDLY at create, so a colliding claim wedges its own Workspace rather than replacing the incumbent's rules. |
   | `block` + `action_parameters.response` in `http_request_firewall_custom` | ✗ `not entitled to use a custom response` — and note the asymmetry: the SAME account gets a custom 429 body in the `http_ratelimit` phase (gotcha 6). Entitlement is per phase, not per plan. |
   | `block`, no `action_parameters`, `starts_with(http.request.uri.path, …)` in the expression | ✓ created; serves Cloudflare's 403 block page. `starts_with()` is entitled on Free (unlike `matches`, which needs a paid plan). |

   Consequence: FU-206's edge leg carries no structured body, and the CORS preflight rule shipped
