...
**Profile-agnostic default, ruled 2026-09-03 (ADR-123) — ☐ not built (FU-206):** a backend's [operational paths](glossary.md) (`/metrics`, `/healthz` and kin) are non-public on every PublicRoute; the composition will block them at the edge as one more rule in the claim's `http_request_firewall_custom` ruleset, with per-path opt-in on the claim. Until it lands, every claim serves those paths publicly — the oracle gateway answers them before auth by design.
...
| Operational paths blocked at the edge by default (ADR-123; `/metrics`, `/healthz`; per-path opt-in) | ☐ **not built — FU-206.** Product zones only until aggregation (teststuff.net: the phase is the ha mTLS entry point). |
