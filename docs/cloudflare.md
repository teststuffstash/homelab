...
|---|---|---|---|
| **Account admin** (CF name: "Create Additional Tokens", → 2027-01-09) | mint scope: user `API Tokens: Write` + zone/account read — *transitively* everything (it mints the rest) | **KeePass ONLY**, host (`~/Documents/homelab-admin.kdbx`, beside the GitHub org-admin) | the operator, solely to apply `tofu/cloudflare-token` (the mint). Never jail, never cluster. |
