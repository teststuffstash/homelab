...
| 4 | **Helm charts, tofu-managed** — now only **cilium, longhorn, argo-cd, garage** (kube-prometheus-stack, metrics-server and forgejo moved to class 1 on 2026-08-04 via the lever; garage is next, FU-136) | `tofu/*.tf` | `terraform` (part of the 47) | ⚠ **plan only** | Merge deploys nothing until someone runs `tofu apply` from the jail. A path rule can open a **plan-report** PR comment; applying stays human. What remains is close to ADR-005's substrate — which is the point: "tofu = human-applied" becomes a coherent rule rather than a lump |
