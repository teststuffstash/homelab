# Model routing — the archaeology (§M1–§M14)

The fourteen investigation sections that used to sit interleaved with the rules in
[`../agents/model-routing.md`](../agents/model-routing.md). Moved here **verbatim** on
2026-09-17 (Goal #1640 acceptance 7): a reader had no way to tell a rule the code enforces from a
proposal somebody wrote in July, and the search that produced the rules is not what a dispatcher
needs. The `§M*` codes are STABLE and remain the anchor other docs and code cite (`§M1a`,
`§M7 leg 5`, `§M12`, …) — this file is where they resolve.

**Nothing here is normative.** The rules are the sibling doc's, each carrying its enforcement
anchor or an explicit `unenforced` mark. Only the relative links inside the moved text were
re-based (`../agents/`) for this file's depth; no prose changed.

⚠ **One dated ruling in here has been overtaken by its own code state**: §M1a's strike-enforcement
knob record still reads as a decision whose deletion is PENDING. It is not — the branch was
deleted and `/route` enforces the task's struck cells unconditionally (homelab#1666 / PR#1685,
assembled to master in PR#1734). The line is kept verbatim because it is the ruling as ruled; the
current rule lives in
[`../agents/model-routing.md`](../agents/model-routing.md) under "The strike ladder".
...
[`roles.md`](../agents/roles.md)); its routing content stays here. A weekly reflex
