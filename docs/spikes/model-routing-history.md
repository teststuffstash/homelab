# Model routing — investigation history and design evolution

Archive of the investigation sections and design journey that shaped `docs/agents/model-routing.md`.
See that doc for the current rules, enforcement anchors, and unenforced marks.
...
## Mechanism — investigation sections (docs/spikes/model-routing-history.md §M1 through docs/spikes/model-routing-history.md §M14)
...
fleet-fault rule — [observability-and-retro.md](../agents/observability-and-retro.md) §Part A⁗), and on
...
falls to the subscription inside `/route` when the OpenRouter rail is capacity-blocked — docs/spikes/model-routing-history.md §M12 is
...
[`roles.md`](../agents/roles.md)); its routing content stays in the main doc. A weekly reflex
...
surface (docs/spikes/model-routing-history.md §M8 classes, docs/spikes/model-routing-history.md §M9 chainless stacks, docs/spikes/model-routing-history.md §M13 research pools). Legs, in build order (FU-161):
...
3. **The canary is a RAIL probe, not a capability probe.** Capability comes from the benchmark
   feed (docs/spikes/model-routing-history.md §M8 feed 1, already pulled weekly by the proxy); the canary answers the one question no
...
4. **Typed verdicts, two sanity rules.** The verdict carries the launcher's `error_class`, not a
   ...
   **Evidence-bearing vs non-evidence partition (FU-161 filing gate, #877).** The shipped
   ...
   - **Evidence-bearing**: `clean` — the model completed a real tool-calling ride — or any
     ...
     `clean` is in practice the only reachable evidence-bearing verdict — the docs/spikes/model-routing-history.md §M1 model-outcome
...
5. **Pool curation — the docs/spikes/model-routing-history.md §M13 duty.** The scout maintains the named class pools: ranked,
...
role ([`roles.md`](../agents/roles.md)) × `task/*` label (FU-114 — one label, N consumers) × repo-type
...
policy: rail affinity (a cache-heavy profile prices brutally on the [Go rail](../agents/chainless-redesign.md)'s list-on-raw draw
...
- ⚠ **A goal small enough for one ride is not a goal** (same ruling era): circles#17
  ...
  ([research-and-specs.md](../agents/research-and-specs.md) step 5 cites this as its evidence).
...
capacity gates, so a routed dispatch still costs at most one read of each (docs/spikes/model-routing-history.md §M11's "no new probes"):
...
the coordinator/reviewer safety net" — docs/spikes/model-routing-history.md §M10). The separation survives; what changes is that it is
...
budgets on [the platform stack](../agents/agentstack.md) stay cap-phantom until SUBSCRIPTION budgets are built (the "get
...
**docs/spikes/model-routing-history.md §M11 above is the general case; this is the emergency one, and it is what actually shipped**
...
from eating the safety net. ⚠ #159 shipped SHADOW-only (docs/spikes/model-routing-history.md §M11a), so **this branch is still the one
...
| one model is 429ing / struck | `cooldown:*` / `strike` | unchanged (docs/spikes/model-routing-history.md §M1) | a bad model is not a dead provider |
...
The research process ([`research-and-specs.md`](../agents/research-and-specs.md)) selects models unlike any
...
- `/route` consumes three more fields: **`class`** (already in the schema — selects the curated
  ...
  tie-break, on the served decision and the docs/spikes/model-routing-history.md §M11 shadow ladder alike. Response also echoes
  `jitter`, and the proxy log carries `draw=<pool>#<slot>@<version>`.
- **Pools are curated, never computed at request time** (docs/spikes/model-routing-history.md §M7 leg 5, scout duty): ranked,
  ...
  [`research-and-specs.md`](../agents/research-and-specs.md), not here.
...
strike-walk (`agents/stacks.json`, coordinator README docs/spikes/model-routing-history.md §MODEL); availability and price already exist
(docs/spikes/model-routing-history.md §M3 registry, docs/spikes/model-routing-history.md §M4 pinning). The new axis: resolve the chain **per task class** (first
approximation: `agent-budget/*` × `track/*` labels) against the registry + strike/ledger history —
the exact gap docs/spikes/model-routing-history.md §M2 notes ("failure classes are task-shaped… should carry a task-size/class
dimension"), with the FU-057 pivot as the data seam. The first axis of the class is **repo-type**:
an `-iac` devops chain is not an app coding chain ([`iac-lane.md`](../agents/iac-lane.md) docs/spikes/model-routing-history.md §Model class).
...
> classifier-based, and none can read our ledger; the docs/spikes/model-routing-history.md §M6 verdict on `auto` stands) or gateway
...
> filter in the docs/spikes/model-routing-history.md §M4 pin (the `/endpoints` field — never pin an fp8/stale serving for
...
> OpenRouter rail. docs/spikes/model-routing-history.md §M6 was re-assessed (the 07-09 no-tools parking was stale). Panel pinned via
...
the live tension between docs/spikes/model-routing-history.md §M2 ("free entries fine anywhere — failure = one strike") and
...
|---|---|---|
| **FU-062** (model-routing) | strikes vs rounds, chains, registry, scout | the single-model lock stands |
