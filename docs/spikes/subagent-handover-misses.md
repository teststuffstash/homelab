...
| 2026-09-14 | #1648 | S8 #1424 residue — IL-T12/IL-T15 theme rows + ROADMAP S8 DONE + acceptance grep (default model, 151k tok/46 calls/40m) | subagent | 0 | 1 (a foreign bookkeeping commit riding the branch — the seat's own unpushed `ba6574be`, inherited by the clone) | 0 (merged) | **a clone of the shared checkout inherits its UNPUSHED master commits, and its `origin` IS the shared checkout, not GitHub**: the dispatch prompt names the GitHub remote explicitly (`git remote add gh …`), the branch is cut from `gh/master`, never local master; the seat's prompt carried the wrong remote fact (corrected mid-task by message) |
...
| 2026-09-05 | #1446 | S8 original 6a: reflex pick + updater pick per (repo, base) lane + the BEHIND first-review admission (opus slot, served model unrecorded; 249k tok/119 calls/35m) | subagent | d4f9c37d | 0 content · **1 prompt-premise error caught by the AUTHOR**: the dispatch named `reviewer-currency/behind-skips` as the fixture pinning the reflex's BEHIND line — it pins the LAUNCHER's spawn-time currency gate; the subagent read both, narrowed the launcher gate too (an out-of-scope file, correctly) and moved the assertion instead of deleting it · 1 beyond-spec catch by the author (the decorrelate flag leaked across dispatches) | (pending) | (lag) | (a) a dispatch that names a fixture as evidence names a CLAIM — say what the fixture pins ("the launcher gate at reviewer-session.sh:202") or say "find which fixture pins X"; (b) a gate with two halves (predicate + spawn-time re-check) needs BOTH halves named in a dispatch that relaxes it, or the relaxation dies at the second half; (c) "run pr-wait in background fashion" still produced a park-as-terminal report cycle (3 notifications) — the next card revision says: report ONCE at PR-open+armed, then EXIT; the seat's standing watch owns the terminal | s3://jail-transcripts/projects/-workspace-homelab/64a7baf3-3c24-4a0c-a133-9d23aa2242ed |
| 2026-09-05 | #1449 | S8 original 6b: scan per-(repo, base) walk + #829 within-lane aging + per-lane famine gauges (opus slot, served model unrecorded; 386k tok/211 calls/92m) | subagent | d4f9c37d | 0 content · 2 prompt-premise errors caught by the AUTHOR: (a) `<!-- agent-event kind=dispatch -->` named as the loss evidence — `mc_event` in code emits only `stats` (the coordinator brief's claim step emits `dispatch`; the author made the count kind-agnostic, under-counting only); (b) the wake stamps described as "keyed per (clause, unit) groups" — they ride the main project/role push; the author put `base` in the GROUP key for the eviction reason · CI: pin-vacuity red on six fixtures whose only diff was `set -u` bridge scaffolding — fixed by cause (scaffolding removed, fixtures reverted verbatim), plus a DIRTY merge of master resolved keeping both sides | (pending) | (lag) | (a) a dispatch that names a marker/metric shape names a CLAIM — cite the emitter file+line or say "find the emitter"; (b) a `block:`-composed fixture must not need bridge defaults for variables the block reads — guard at the CALL SITE (`${var:-}`) so the pin-vacuity gate never sees scaffolding as a pin; (c) the report-once-then-exit instruction WORKED (one notification) — promote it to the card | s3://jail-transcripts/projects/-workspace-homelab/64a7baf3-3c24-4a0c-a133-9d23aa2242ed |
