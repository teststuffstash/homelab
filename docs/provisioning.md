...
**A node the reconciler owns** — `reconcile: auto` in `machines/machines.yaml`, and today, with
`reconcile_rollout.enabled` off, only its `pilot` (`wk-03`) — needs none of this: the box's
