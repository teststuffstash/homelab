...
| Laptops | **control planes** — low idle draw, small SSD, no other job | ✅ `wk-metal-02` (X250) is a control plane since 2026-09-22 (ADR-133); the third CP went to a VM (cp-02 on nx-02), so `wk-metal-03` (X260) stays a ride node. ⚠ `wk-metal-01` (X240) is also a laptop but cannot move: it is a Garage zone + the bulk MX500 |
...
| [The management box](docs/management-box.md) (`thinkcentre`) | R12 out-of-band applier — no workloads, no storage (ADR-129) | 🔜 pilot installed 2026-09-13 (`docs/management-box.md` §MB1); its test surface is the main root's raw-k8s residue, kept in tofu for it (operator, 2026-09-13) — then Renovate through the box (G-D), then the ArgoCD migration + the second OPNsense / third CP for it to operate on |
...
| # | Goal | scope pointers |
|---|---|---|
| G-A | **every role routes** — the chainless completion. **LAUNCHED 2026-08-23 → homelab#775** (status lives there) | charter build items 3–6 + flip acceptance 2–4; #516 family decorrelation; FU-127; FU-095 (a)/(c); ex-FU-179 strike policy (ruled); FU-180 relates (accounting half) |
...
| G-D | **Renovate live for the platform stack** — charter drafted and preserved in closed #502; re-mint at launch. First class-6 (cluster-substrate, human-applied) ride = the Talos 1.13.2→1.13.8 patch, the #857 consumer (operator, 2026-09-01: parked on this launch, not a separate sitting). **Applies ride the management box** (operator, 2026-09-13): the stint after the box proves phase B on its test surface — `docs/management-box.md` §The test surface. **Ordering ruled 2026-09-18 (operator): the ROLLOUT is automated FIRST.** An automatic version-bump PR buys nothing while landing it still costs a manual sitting — so the node-upgrade verb and the box-run rollout (ADR-132's `reconcile: auto`, WIP 1) are the PREREQUISITE for Renovate on class 6, not the other way round. The three converging enablers are now all in hand: the verb, the management box, and enough hardware for ADR-133's three control planes (FU-243). Detection of "we are behind / out of support" SHIPPED 2026-09-23 — the box belt's substrate check (`docs/management-box.md` §MB2; FU-254, archived). **Before the Goal (operator, 2026-09-21): a few manual, ATTENDED version bumps through the box path, one with a deliberate rollback** — "letting renovate create 20 PR-s is too much for untested flows". **RE-HEADED 2026-09-25 as stint S9 → homelab#1985** (the #502 charter is the parent body; blast classes, the migration lens, the workflow-pin carve-out, the 7-column coverage table — status lives there) | homelab#1985 (stint S9), homelab#502 (closed, body = the draft), #857 |
...
| G-H | **the router routes, the launcher retries** — provider-axis strikes, per-session pins, no coordinator tax on an empty ride. **LAUNCHED 2026-09-13 → homelab#1640** (themed, the FIRST live ADR-126 theme: #1641 `router` on `goal/1640-router`; UNQUEUED until the #1231 verdict + S8 read, operator order) | charter ⚖ router-first corollary (b) rung-0 un-banked for the salvage-none case; FU-186/200/201/202/213; #1231 out-of-scope list; 2026-09-13 five-strike world |
