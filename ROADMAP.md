...
| S6 | epic post-deploy mechanic (the 2026-08-20 lineage sitting): the contract's post-deployment rule + reviewer depth logic (lane split, depth≥4 pushback, mint-to-origin — code-first) + **bind-at-filing at EVERY authoring surface** (the ADR-109 author-surface inventory is the checklist: responder → diagnosed cause; worker-filed siblings → the ride's own issue, per-repo recipe paste; retro → per-finding origin; arbitrate/coordinator plays + seat card → epic ancestor; harvest/checkpoint already bind) + the unbound-sprout report belt as backstop. Ships contract+code as one unit; dogfoods itself; #628 leg 3 is the measurement twin | FU-090 (its depth-gate Next), TICK-LOG 2026-08-20 |
