...
# is detected. See docs/agents/merge-path.md + docs/github-setup.md §4.
#
# RETIRED 2026-08-26 (ADR-111 cutover, homelab#745): the MERGE_GH_APP_ID / MERGE_GH_APP_PRIVATE_KEY org
# secrets are gone — the hosted update-pr-branch reusable + its per-repo callers are deleted, and the
# in-cluster updater (agents/coordinator/update-pr-argo.yaml) gets the homelab-merge App key via
# Infisical→ESO (agents/coordinator/updater-git.yaml), never through the CI plane. Removing the org
# secrets closes the CI-plane exposure the dedicated App was created to contain. The App itself LIVES ON
# as the updater's identity; its key's durable source stays Infisical (MERGE_GH_APP_PRIVATE_KEY).
# The next host-side `devbox run github-tofu apply` destroys the two org secrets.
